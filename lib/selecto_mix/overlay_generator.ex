defmodule SelectoMix.OverlayGenerator do
  ...
  defp generate_column_examples(config) do
    columns = get_in(config, [:source, :columns]) || %{}
    ...
    examples =
      ...
      |> Enum.take(3)
      |> Enum.map(fn {field_name, column_config} ->
        generate_column_example(field_name, column_config)
      end)
      |> Enum.join("\n")
    ...
    if examples == "" do
      "          #   :field_name => %{\n" <>
        "          #     label: \"Custom Label\",\n" <>
        "          #     format: :currency,  # or :percentage, :number, etc.\n" <>
        "          #     aggregate_functions: [:sum, :avg, :min, :max]\n" <>
        "          #   }"
    else
      examples
  ...
  defp generate_column_example(field_name, column_config) do
    type = Map.get(column_config, :type, :string)
    ...
    String.trim_trailing(example)
  end

  defp generate_filter_examples(config) do
    columns = get_in(config, [:source, :columns]) || %{}
    ...
    example_field =
      columns
      |> Enum.find(fn {_field, col_config} ->
        Map.get(col_config, :type) in [:string, :integer, :decimal]
      end)
