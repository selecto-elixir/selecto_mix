defmodule SelectoMix.JoinAnalyzer do
  ...
  def analyze(schema_module, opts \\ []) do
    ...
    optimized_joins = optimize_joins(Map.merge(basic_joins, many_to_many), opts)

    %{
      joins: Map.merge(basic_joins, optimized_joins),
      relationships: categorize_relationships(schema_info.associations, many_to_many),
  ...
  def analyze_many_to_many(schema_module) do
    try do
      ...
      |> Enum.filter(fn {_name, assoc} ->
        # Check if it's a many_to_many association
        assoc.__struct__ == Ecto.Association.ManyToMany
      end)
      |> Enum.map(fn {assoc_name, assoc} ->
  ...
  def detect_circular_dependencies(joins, max_depth) do
    ...
    graph = Enum.reduce(joins, %{}, fn {from, config}, acc ->
      to = config[:schema] || config.schema
      # Handle both atoms and module names
      to_key = if is_atom(to) and function_exported?(to, :__schema__, 1) do
        to |> Module.split() |> List.last() |> String.downcase() |> String.to_atom()
      else
        to
      end
      Map.update(acc, from, [to_key], &[to_key | &1])
    end)
  ...
  defp analyze_hierarchy_type(assoc, schema_info) do
    cond do
      # Adjacency list pattern (parent_id, referrer_id)
      assoc[:foreign_key] in [:parent_id, :parent, :referrer_id] ->
        %{
          type: :adjacency_list,
          field: assoc.name,
  ...
  defp build_many_to_many_condition(assoc) do
    case assoc.join_keys do
      # Handle keyword list format: [user_id: :id, group_id: :id]
      [{key1, val1}, {key2, val2}] ->
        [{val1, key1}, {key2, val2}]

      # Handle tuple format (fallback)
      {key1, key2} when is_atom(key1) and is_atom(key2) ->
        [{:id, key1}, {key2, :id}]

      # Handle list of atoms (fallback)
      [key1, key2] when is_atom(key1) and is_atom(key2) ->
        [{:id, key1}, {key2, :id}]

      # Default fallback
      _ ->
        [{:id, :owner_id}, {:related_id, :id}]
    end
  end
  ...
  defp categorize_relationships(associations, many_to_many) do
    basic_relationships = associations
    ...
    # Add many-to-many relationships if any exist
    if map_size(many_to_many) > 0 do
      Map.put(basic_relationships, :many_to_many, Map.keys(many_to_many))
    else
      basic_relationships
    end
  end
  ...
  defp generate_warnings(cycles, hierarchies) do
    ...
    # Check for hierarchical relationships that could form cycles
    warnings = if Enum.any?(hierarchies, &(&1.type == :adjacency_list)) do
      ["Circular dependency detected: Hierarchical relationships may form cycles" | warnings]
    else
      warnings
    end
